package kz.spring.course.course;

import kz.spring.course.course.entity.Currency;
import kz.spring.course.course.service.CurrencyConversionService;
import kz.spring.course.course.service.CurrencyModeService;
...
public class TestBot extends TelegramLongPollingBot {
    ...
    public String getBotToken() {
        return "5206425584:AAH9Gtyh5Qqiw7GZvX98KYtojw4C4TI5mpY";
    }
        ...
        if (message.hasText()){
            ...
            if (value.isPresent()){
                execute(SendMessage.builder()
                        .chatId(message.getChatId().toString())
                        .text(String.format(
                                "%4.2f %s is %4.2f %s",value.get(),originalCurrency,(value.get()*ratio),targetCurrency))
                        .build());
                return;
            }
        }
    ...
    private String getCurrencyButton(Currency seved, Currency current){
        return seved==current ? current+"✅":current.name();
    }
